...
function Evaluation({ bundle }: { bundle: ReplayBundle }) {
  ...
  return <div className="mc-evaluation">
    ...
    <details className="mc-disclosure" open><summary>Evaluation scope <span>Read before interpreting the scores</span></summary><div className="mc-scope-grid"><p><strong>Balanced sample</strong>123 positive + 123 nominal windows. Precision on this curated cohort is not deployment precision.</p><p><strong>Pair-based split</strong>80/10/10 by event-channel-occurrence pair. {bundle.run.split.testEventIdsAlsoInTrain} of {bundle.run.split.testEventIds} test event IDs also occur in training.</p><p><strong>Generalization limit</strong>These scores describe the saved split. They do not establish performance on unseen events or a chronological holdout.</p></div></details>
  </div>;
}
