...
class CurriculumTrainer:
    ...
    - stage6_esa_cot: Trains the model on ESA-AD Mission1 subsystem_5 anomaly detection with
    ...
    def _load_previous_stage_model(
        ...
        try:
            ...
            if not os.path.exists(metrics_file):
                # PATCH: stage6_esa_cot is a standalone fine-tune (not a continuation of the
                # published 5-stage curriculum), so it always starts fresh instead of chaining
                # from stage5_ecg_cot.
                # PATCH: If running stage2_captioning and previous stage metrics are missing, skip loading
                if current_stage in ("stage2_captioning", "stage6_esa_cot"):
                    if self.rank == 0:
                        print(
    ...
    def _train_stage(
        ...
        try:
            ...
            if previous_stage_info:
                ...
                # Only allow fresh model for the first stage, or for stage6_esa_cot, which is a
                # standalone fine-tune independent of the published 5-stage curriculum.
                if stage_name != CURRICULUM_STAGES[0] and stage_name != "stage6_esa_cot":
                    raise RuntimeError(
    ...
    def stage6_esa_cot(
        ...
        """Stage 6: Chain-of-Thought Reasoning (ESA-AD Mission1 subsystem_5 anomaly detection).
        ...
        return self._train_stage(
            ...
            dataset_class=ESAMission1CoTQADataset,
            num_epochs=30,
            lr_encoder=2e-4,
            lr_projector=1e-4,
            lr_base=2e-4,
    ...
    def run_curriculum(
        ...
        for stage in incomplete_stages:
            ...
            if stage == "stage1_mcq":
                ...
                results[stage] = stage_results
                self._mark_stage_completed(stage, stage_results)
            elif stage == "stage6_esa_cot":
                stage_results = self.stage6_esa_cot(
                    batch_size=batch_size, eval_only=eval_only
                )
                results[stage] = stage_results
                self._mark_stage_completed(stage, stage_results)
    ...
    def _enable_lora_if_needed(self, stage_name: str):
        ...
        stages_with_lora = ["stage3_cot", "stage4_sleep_cot", "stage5_ecg_cot", "stage6_esa_cot"]
